...
const runId = `live_recording_${Date.now()}_${crypto.randomBytes(4).toString('hex')}`;
...
function seedSqlite(dbPath) {
  ...
  db.prepare(`INSERT INTO workspaces (id, name, status, admin_email, created_at) VALUES (?, ?, 'approved', ?, datetime('now'))`)
    .run(ids.workspaceId, 'Live Recording Workspace', `teacher.${runId}@example.com`);
  db.prepare(`INSERT INTO workspaces (id, name, status, admin_email, created_at) VALUES (?, ?, 'approved', ?, datetime('now'))`)
  ...
  db.prepare(`
    ...
    ) VALUES (?, ?, ?, 'Consent Recording Class', '2099-01-10', '10:00', '11:00', ?, '', '', 'scheduled', 'channel', NULL, ?, datetime('now'), datetime('now'), '')
...
async function main() {
  ...
  try {
    ...
    await api(baseUrl, studentAJar, 'POST', `/api/live-sessions/${encodeURIComponent(ids.sessionId)}/request-join`, {
      expectedStatus: 202
    });
    ...
    await api(baseUrl, studentAJar, 'POST', `/api/live-sessions/${encodeURIComponent(ids.sessionId)}/join`);

    await api(baseUrl, studentBJar, 'POST', `/api/live-sessions/${encodeURIComponent(ids.sessionId)}/request-join`, {
      expectedStatus: 202
    });
    await api(baseUrl, teacherJar, 'POST', `/api/live-sessions/${encodeURIComponent(ids.sessionId)}/participants/${encodeURIComponent(ids.studentBId)}/approve`);

    await api(baseUrl, teacherJar, 'POST', `/api/live-sessions/${encodeURIComponent(ids.sessionId)}/start-recording`);

    const teacherState = await api(baseUrl, teacherJar, 'GET', `/api/live-sessions/${encodeURIComponent(ids.sessionId)}/state`);
    assert.strictEqual(teacherState.liveControls.recording.enabled, true, 'teacher should see recording enabled');
    assert.strictEqual(teacherState.liveControls.counts.consentPending, 2, 'two students should be pending recording consent');

    const studentAState = await api(baseUrl, studentAJar, 'GET', `/api/live-sessions/${encodeURIComponent(ids.sessionId)}/state`);
    assert.strictEqual(studentAState.liveControls.recording.selfConsentRequired, true, 'joined student should require consent once recording starts');

    const blockedJoin = await api(baseUrl, studentAJar, 'POST', `/api/live-sessions/${encodeURIComponent(ids.sessionId)}/join`, {
      expectedStatus: 403
    });
    assert.strictEqual(blockedJoin.code, 'live_recording_consent_required', 'student should be blocked until consent is given');

    const studentStart = await api(baseUrl, studentAJar, 'POST', `/api/live-sessions/${encodeURIComponent(ids.sessionId)}/start-recording`, {
      expectedStatus: 403
    });
    assert.strictEqual(studentStart.error, 'Forbidden', 'student should not start recording');

    const outsiderConsent = await api(baseUrl, outsiderJar, 'POST', `/api/live-sessions/${encodeURIComponent(ids.sessionId)}/recording-consent`, {
      json: { consent: true },
      expectedStatus: 403
    });
    assert.strictEqual(outsiderConsent.code, 'tenant_forbidden', 'cross-tenant consent should be denied');

    const consentedA = await api(baseUrl, studentAJar, 'POST', `/api/live-sessions/${encodeURIComponent(ids.sessionId)}/recording-consent`, {
      json: { consent: true }
    });
    assert.strictEqual(consentedA.liveControls.self.recordingConsent, true, 'student A should persist recording consent');

    const joinedAgainA = await api(baseUrl, studentAJar, 'POST', `/api/live-sessions/${encodeURIComponent(ids.sessionId)}/join`);
    assert.strictEqual(joinedAgainA.ok, true, 'consented student should be able to continue joining');

    const declinedB = await api(baseUrl, studentBJar, 'POST', `/api/live-sessions/${encodeURIComponent(ids.sessionId)}/recording-consent`, {
      json: { consent: false }
    });
    assert.strictEqual(declinedB.code, 'live_recording_consent_denied', 'decline path should be explicit');

    const blockedB = await api(baseUrl, studentBJar, 'POST', `/api/live-sessions/${encodeURIComponent(ids.sessionId)}/join`, {
      expectedStatus: 403
    });
    assert.strictEqual(blockedB.code, 'live_recording_consent_required', 'declining consent should block later join attempts');

    const adminJoin = await api(baseUrl, adminJar, 'POST', `/api/live-sessions/${encodeURIComponent(ids.sessionId)}/join`);
    assert.strictEqual(adminJoin.ok, true, 'same-workspace admin should still bypass consent gating');

    const stopped = await api(baseUrl, teacherJar, 'POST', `/api/live-sessions/${encodeURIComponent(ids.sessionId)}/stop-recording`);
    assert.strictEqual(stopped.liveControls.recording.enabled, false, 'teacher should be able to stop recording consent flow');

    console.log('[live-recording-consent-smoke] passed');
  } finally {
    await stopServer(child);
    fs.rmSync(uploadsDir, { recursive: true, force: true });
  }
...
main().catch((err) => {
  console.error('[live-recording-consent-smoke] failed:', err && err.stack ? err.stack : err);
  process.exit(1);
});
